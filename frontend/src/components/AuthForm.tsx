    ...
    try {
      if (mode === 'login') {
        ...
        addToast({
          ...
          description: 'Теперь вы можете войти, используя свои учетные данные.'
        });
        await login(data.username, data.password);
      }
      ...
      addToast({
        ...
        description: error.message || 'Пожалуйста, проверьте свои учетные данные и попробуйте снова.'
      });
    } finally {
      setIsLoading(false);
    }
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl p-8 fade-in-up">
          <CardHeader className="text-center p-0 mb-6">
            ...
            <CardDescription className="text-lg text-muted-foreground mb-2 font-sans">
              {mode === 'login' ? 'Войдите в свой аккаунт' : 'Присоединяйтесь к NeuroBeton сегодня'}
            </CardDescription>
          </CardHeader>
              ...
              >
                {isLoading ? (
                  ...
                  <>
                    {mode === 'login' ? (
                      <>
                        <ArrowRightIcon className="w-5 h-5 mr-2" /> Войти
                      </>
                    ) : (
                      <>
                        <UserPlusIcon className="w-5 h-5 mr-2" /> Регистрация
                      </>
                    )}
                  </>
                )}
